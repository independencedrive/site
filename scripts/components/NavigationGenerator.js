...
export class NavigationGenerator {
  ...
  static generateMegaMenuContent(item) {
    ...
    return `
      <div class="megamenu-dropdown absolute top-full mt-0 bg-white shadow-2xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 border-t-4 border-red-600 rounded-b-lg z-50 ${isFormations || isAutresPrestations || isCodeDeLaRoute ? 'w-[600px] max-w-[90vw]' : isSingleCategory ? 'min-w-[320px] max-w-[420px] mx-auto' : 'w-screen max-w-[min(90vw,1152px)]'}" 
           style="left: 50%; transform: translateX(-50%);">
        <div class="p-6">
          ${isFormations ? this.generateFormationsLayout(item) : isAutresPrestations ? this.generateAutresPrestationsLayout(item) : isCodeDeLaRoute ? this.generateCodeDeLaRouteLayout(item) : this.generateStandardLayout(item)}
  ...
  // Generate layout for "Code de la route" menu
